...
/**
 * Fetch Marketplace revenue summary for the authenticated user.
 * Requires authentication.
 *
 * @param {object} _params
 * @param {{ cache: import('../cache.js').MemoryCache, getToken: () => Promise<string|null>, getBaseUrl: () => string }} ctx
 */
export async function marketplaceEarnings(_params, ctx) {
	const token = await ctx.getToken()
	if (!token) {
		return errorResponse(ERROR_MESSAGES.noToken)
	}

	const cacheKey = 'marketplace-earnings'

	const cached = ctx.cache.get(cacheKey)
	if (cached) return cached

	const baseUrl = ctx.getBaseUrl()
	const result = await registryGet('/marketplace/earnings', token, baseUrl)
