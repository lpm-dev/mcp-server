 ...
 *
 * @param {{ query?: string, category?: string, ecosystem?: string, limit?: number }} params
 * @param {{ cache: import('../cache.js').MemoryCache, getToken: () => Promise<string|null>, getBaseUrl: () => string }} ctx
 */
export async function marketplaceSearch({ query, category, ecosystem, limit }, ctx) {
	if (!query && !category) {
	...
	const cacheKey = `marketplace:${query || ''}:${category || ''}:${ecosystem || ''}:${limit || 10}`
