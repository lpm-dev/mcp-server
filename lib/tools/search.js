 ...
 *
 * @param {{ query: string, ecosystem?: string, limit?: number }} params
 * @param {{ cache: import('../cache.js').MemoryCache, getToken: () => Promise<string|null>, getBaseUrl: () => string }} ctx
 */
export async function search({ query, ecosystem, limit }, ctx) {
	...
	const cacheKey = `search:${q}:${ecosystem || ''}:${safeLimit}`
	...
	for (const pkg of packages) {
		...
		const ecosystemLabel =
			pkg.ecosystem && pkg.ecosystem !== 'js'
				? ` {${pkg.ecosystem}}`
				: ''
		lines.push(`- ${name}${typeLabel}${ecosystemLabel}${desc}${downloads}`)
	}
