...
export async function search(params, ctx) {
	const {
		query, category, ecosystem, distribution, packageType,
		sort, hasTypes, moduleType, license, minNodeVersion, limit,
	} = params
	...
	const hasStructuredFilters = category || distribution || packageType
		|| sort || hasTypes || moduleType || license || minNodeVersion
...
async function semanticSearch({ query, ecosystem, limit }, token, baseUrl) {
	...
	const result = await searchGet(`/packages?${searchParams.toString()}`, token, baseUrl)
...
function formatResults(packages, query) {
	...
	for (const pkg of packages) {
		...
		const typeLabel = pkg.packageType && pkg.packageType !== 'package'
			? ` [${pkg.packageType}]`
			: ''

		// Ecosystem badge (skip default "js")
		const ecosystemLabel = pkg.ecosystem && pkg.ecosystem !== 'js'
			? ` {${pkg.ecosystem}}`
			: ''

		// Distribution mode
		const distLabel = pkg.distributionMode
			? ` (${pkg.distributionMode})`
			: ''
		...
		lines.push(`- ${name}${typeLabel}${ecosystemLabel}${distLabel}${desc}${downloads}`)
