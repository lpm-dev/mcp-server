...
export function resolveCli(platform = process.platform) {
	...
	if (envPath) {
		const syntax = platform === "win32" ? path.win32 : path.posix
		const qualified =
			syntax.isAbsolute(envPath) &&
			(platform !== "win32" || syntax.parse(envPath).root.length > 1)
		return qualified ? envPath : null
	}
