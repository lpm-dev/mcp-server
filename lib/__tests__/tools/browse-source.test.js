...
describe('browseSource tool', () => {
	...
	it('returns file tree when no path specified', async () => {
		registryGet.mockResolvedValueOnce({ ok: true, status: 200, data: MOCK_TREE_RESPONSE })
	...
	it('returns tree + file contents when path specified', async () => {
		registryGet.mockResolvedValueOnce({ ok: true, status: 200, data: MOCK_FILES_RESPONSE })

		const result = await browseSource({ name: 'alice.ui-kit', path: 'src' }, createContext())
		const data = JSON.parse(result.content[0].text)
	...
	it('calls registry API with version and path params', async () => {
		...
		await browseSource({ name: 'alice.ui-kit', version: '1.0.0', path: 'src' }, createContext())
	...
	it('returns error for 404 (version not found)', async () => {
		...
		const result = await browseSource({ name: 'alice.ui-kit', version: '99.0.0' }, createContext())
		expect(result.isError).toBe(true)
	...
	it('does not cache different paths', async () => {
		registryGet.mockResolvedValue({ ok: true, status: 200, data: MOCK_TREE_RESPONSE })
		const ctx = createContext()
