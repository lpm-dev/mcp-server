...
describe("getInstallCommand tool", () => {
	...
	it("returns lpm install for Swift packages", async () => {
		registryGet.mockResolvedValueOnce({
			ok: true,
			status: 200,
		...
		expect(data.command).toBe("lpm install @lpm.dev/alice.swift-lib")
		expect(data.method).toBe("install")
	})

	it("returns lpm install for xcframework packages", async () => {
		registryGet.mockResolvedValueOnce({
			ok: true,
			status: 200,
		...
		expect(data.command).toBe("lpm install @lpm.dev/alice.my-framework")
		expect(data.method).toBe("install")
	})
	...
	it("returns fallback lpm install when API returns non-404 error", async () => {
		...
		expect(result.content[0].text).toBe("lpm install @lpm.dev/alice.ui-kit")
	})
