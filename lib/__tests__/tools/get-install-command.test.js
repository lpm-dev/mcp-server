...
describe('getInstallCommand tool', () => {
	...
	it('returns lpm install for plain JS packages', async () => {
		...
		const result = await getInstallCommand({ name: 'alice.ui-kit' }, createContext())
	...
	it('returns lpm add for non-package types (mcp-server)', async () => {
		...
		const result = await getInstallCommand({ name: 'alice.my-mcp' }, createContext())
		const data = JSON.parse(result.content[0].text)
	...
	it('returns lpm add for Swift packages', async () => {
		...
		const result = await getInstallCommand({ name: 'alice.swift-lib' }, createContext())
		const data = JSON.parse(result.content[0].text)
	...
	it('returns lpm add for xcframework packages', async () => {
		...
		const result = await getInstallCommand({ name: 'alice.my-framework' }, createContext())
		const data = JSON.parse(result.content[0].text)
	...
	it('includes version when provided', async () => {
		...
		const result = await getInstallCommand({ name: 'alice.ui-kit', version: '2.0.0' }, createContext())
		const data = JSON.parse(result.content[0].text)
	...
	it('returns error for invalid name format', async () => {
		const result = await getInstallCommand({ name: 'no-dot-here' }, createContext())
	...
	it('returns error for 404', async () => {
		...
		const result = await getInstallCommand({ name: 'alice.missing' }, createContext())
