...
describe('llmContext tool', () => {
	...
	it('returns full llm context', async () => {
		registryGet.mockResolvedValueOnce({ ok: true, status: 200, data: MOCK_LLM_CONTEXT })
	...
	it('calls correct API endpoint with version', async () => {
		...
		await llmContext({ name: 'alice.ui-kit', version: '2.1.0' }, createContext())
	...
	it('uses different cache keys for different versions', async () => {
		registryGet.mockResolvedValue({ ok: true, status: 200, data: MOCK_LLM_CONTEXT })
		const ctx = createContext()
	...
	it('returns error for 404 with version label', async () => {
		...
		const result = await llmContext({ name: 'alice.missing', version: '9.9.9' }, createContext())
