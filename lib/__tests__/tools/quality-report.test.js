...
describe('qualityReport tool', () => {
	...
	it('returns full quality report', async () => {
		registryGet.mockResolvedValueOnce({ ok: true, status: 200, data: MOCK_QUALITY })

		const result = await qualityReport({ name: 'alice.ui-kit' }, createContext())
	...
	it('returns error for 404', async () => {
		...
		const result = await qualityReport({ name: 'alice.missing' }, createContext())
	...
	it('returns error for 403', async () => {
		...
		const result = await qualityReport({ name: 'alice.private' }, createContext())
