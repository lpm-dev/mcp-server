...
describe("auth", () => {
	...
	describe("registry-scoped keychain accounts", () => {
		...
		it.each([
			...
			"http://[0:0:0:0:0:ffff:127.0.0.42]:8787",
			"http://127.1:8787",
		...
		it("uses the mapped-loopback registry account instead of the hosted account", () => {
			const registryUrl = "http://[::ffff:127.0.0.1]:8787"
			const resolved = resolveRegistryUrl(registryUrl)

			expect(resolved).toBe(registryUrl)
			expect(getKeychainAccount(resolved)).not.toBe(
				getKeychainAccount("https://lpm.dev"),
			)
		})
	})
})
