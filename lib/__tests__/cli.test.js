...
describe("resolveCli", () => {
	...
	it("returns LPM_CLI_PATH env var when set", () => {
		process.env.LPM_CLI_PATH = path.resolve("/custom/path/lpm")
		expect(resolveCli()).toBe(path.resolve("/custom/path/lpm"))
	})
...
describe("runCli", () => {
	beforeEach(() => {
		vi.resetAllMocks()
		process.env.LPM_CLI_PATH = path.resolve("/usr/local/bin/lpm")
	})
	...
	it("passes correct args to execFile", async () => {
		...
		expect(execFile).toHaveBeenCalledWith(
			path.resolve("/usr/local/bin/lpm"),
