...
import { registryGet, searchGet } from "../api.js"
...
describe("Integration: AI workflow", () => {
	...
	it("search → package info → browse source → add (JS package)", async () => {
		...
		registryGet.mockResolvedValueOnce({
			...
			data: {
				...
				accessInfo: {
					model: "pool",
					summary: "Included with Pool subscription",
				},
			},
		})
		...
		const fileResult = await browseSource(
			{ name: "alice.ui-kit", path: "src/button.jsx" },
			ctx,
		)
		...
		const addResult = await add(
			{ name: "alice.ui-kit", path: "src/components/ui" },
			ctx,
		)
	...
	it("search → package info → browse source (Swift package)", async () => {
		...
		const searchResult = await search(
			{ query: "networking", ecosystem: "swift" },
			ctx,
		)
	...
	it("unauthenticated user gets helpful errors from protected operations", async () => {
	...
	it("marketplace access denied shows purchase message", async () => {
		...
		registryGet.mockResolvedValueOnce({
			...
			data: {
				error:
					"Package @lpm.dev/alice.premium requires purchase. Visit the package page to buy access.",
			},
		})
