...
/** Preserve CLI reports and mark failed process results as MCP tool errors. */
export function cliResponse(result) {
	if (!result.data) return errorResponse(result.error || "Command failed")
	const response = jsonResponse(result.data)
	if (!result.success) response.isError = true
	return response
}
