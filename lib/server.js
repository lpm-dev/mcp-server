...
export function createServer() {
	...
	server.tool(
		"lpm_quality_report",
		"Get the quality score and detailed, ecosystem-specific check breakdown for a package on the LPM.dev Registry across documentation, code quality, testing, and health.",
		{
	...
	server.tool(
		"lpm_audit",
		"Run a security audit on the project's LPM dependencies. Returns behavioral tags (eval, childProcess, shell, dynamicRequire), AI security findings, quality scores, and lifecycle scripts. Requires LPM CLI installed; registry access follows CLI authentication and policy.",
		{
			path: z
				.string()
				.optional()
...
If you must use this tool: fetch directory paths (e.g., "src") to get multiple files in one request rather than individual files.`,
	...
	server.tool(
		...
		"Search LPM packages using natural language or structured filters. Uses hybrid semantic search for natural language queries. Supports filtering by category, distribution mode, package type, ecosystem, license, and more.",
		{
			...
			distribution: z
				.enum(["free", "marketplace", "pool", "private"])
				.optional()
